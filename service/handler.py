...
from data_loader import TRACK_POINT_TIME_INTERVAL
...
class IntelligenceHandler(Iface):
    def forecast_xy(self, fn: str, x: float, y: float, h: float, v: float, course: float, dx: float, dy: float) -> int:
        global XY_TRACK_POINTS
        XY_TRACK_POINTS[fn].append(x, y)
    ...
    def forecast_ll(self, fn: str, longi: float, lati: float, h: float, v: float, course: float, dlongi: float, dlati: float) -> int:
        global LL_TRACK_POINTS
        LL_TRACK_POINTS[fn].append(longi, lati)
