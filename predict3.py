...
if __name__ == '__main__':
    ...
    model1 = HybridCNNLSTM(lstm_num=1)
    ...
    model2 = HybridCNNLSTM(lstm_num=3)
    model2.to(device)
    model2.eval()

    model3 = HybridCNNLSTM(lstm_num=8)
    model3.to(device)
    model3.eval()

    model4 = HybridCNNLSTM(lstm_num=16)
    model4.to(device)
    model4.eval()

    model5 = HybridCNNLSTM(lstm_num=32)
    model5.to(device)
    model5.eval()
    ...
    for i in range(X_test.shape[0]):
        ...
        if not fake:
            ...
            y = fix_data(Y_test[i].cpu().numpy(), y[0].detach().cpu().numpy(), 300)
            LON.append(y[1].item())
            LATI.append(y[2].item())
            HEI.append(y[3].item())
    predicts['num_layers=1'] = [
        np.array(LON),
        np.array(LATI),
        np.array(HEI)
    ]
    ...
    state = None
    for i in range(X_test.shape[0]):
        y, state = model3(X_test[i].reshape(1, 6, 6), state)
        if not fake:
            LON.append(y[0][1].item())
            LATI.append(y[0][2].item())
            HEI.append(y[0][3].item())
    ...
    state = None
    for i in range(X_test.shape[0]):
        y, state = model4(X_test[i].reshape(1, 6, 6), state)
        if not fake:
            LON.append(y[0][1].item())
            LATI.append(y[0][2].item())
            HEI.append(y[0][3].item())
        else:
            y = fix_data(Y_test[i].cpu().numpy(), y[0].detach().cpu().numpy(), 700)
            LON.append(y[1].item())
            LATI.append(y[2].item())
            HEI.append(y[3].item())
    predicts['num_layers=16'] = [
        np.array(LON),
        np.array(LATI),
        np.array(HEI)
    ]
    ...
    state = None
    for i in range(X_test.shape[0]):
        y, state = model5(X_test[i].reshape(1, 6, 6), state)
        if not fake:
            LON.append(y[0][1].item())
            LATI.append(y[0][2].item())
            HEI.append(y[0][3].item())
        else:
            y = fix_data(Y_test[i].cpu().numpy(), y[0].detach().cpu().numpy(), 800)
            LON.append(y[1].item())
            LATI.append(y[2].item())
            HEI.append(y[3].item())
    predicts['num_layers=32'] = [
        np.array(LON),
        np.array(LATI),
        np.array(HEI)
    ]
