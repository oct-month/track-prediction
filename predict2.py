...
def show_3D(sources, predicts):
    ...
    ax.plot3D(sources_lon, sources_la, sources_h, c='black', label='真实')
    ax.plot3D(predicts_lon, predicts_la, predicts_h, c='blue', label='预测')
    plt.legend()
    plt.show()
...
def show_2D(sources, predicts, steps=1):
    ...
    plt.plot(sources[0][::steps], sources[1][::steps], c='black', label='真实')
...
if __name__ == '__main__':
    ...
    for pp in os.listdir(DATA_AFTER_DIR):
        ...
        X_test, Y_test = data_X_Y(p, 3300, -2000)
